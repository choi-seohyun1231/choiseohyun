...
#define N 5
#define NAME_LEN 50
#define FILE_NAME "data.bin"  // 파일 이름을 .bin으로 변경하여 바이너리 파일임을 표시
...
void 파일에서_불러오기(int *in, int *sold, int *left, int *totalSold, int *totalIn, int prices[N], char names[N][NAME_LEN]) {
    ...
    fread(totalSold, sizeof(int), 1, file);
    fread(totalIn, sizeof(int), 1, file);
    fread(in, sizeof(int), N, file);
    fread(sold, sizeof(int), N, file);
    fread(left, sizeof(int), N, file);
    fread(prices, sizeof(int), N, file);
    fread(names, sizeof(char), N * NAME_LEN, file);
...
void 파일에_저장하기(int *in, int *sold, int *left, int totalSold, int totalIn, int prices[N], char names[N][NAME_LEN]) {
    ...
    fwrite(&totalSold, sizeof(int), 1, file);
    fwrite(&totalIn, sizeof(int), 1, file);
    fwrite(in, sizeof(int), N, file);
    fwrite(sold, sizeof(int), N, file);
    fwrite(left, sizeof(int), N, file);
    fwrite(prices, sizeof(int), N, file);
    fwrite(names, sizeof(char), N * NAME_LEN, file);
